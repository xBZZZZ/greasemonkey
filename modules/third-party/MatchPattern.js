...
MatchPattern.prototype.doMatch = function(uriSpec) {
  ...
  if (this._host) {
    if (this._host.match(gTldHostRegExp)) {
      var tldBase = null;
      try {
        tldBase = gTldService.getPublicSuffix(matchURI);
      } catch (e) {
      ...
      if (tldBase) {
        ...
        if (!tldWhitelistEnabled || (tldWhitelistEnabled && tldWhitelist)) {
          this._host = this._host.replace(gTldHostRegExp, '$1.' + tldBase);
        } else {
